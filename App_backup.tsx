...
const ProfileDropdown = ({ navigateTo, isUserMenuOpen, setIsUserMenuOpen, user, profileData, setShowProductManagement, setOpenAddForm }: { 
    ...
    const handleMouseLeave = () => {
        console.log('DROPDOWN MOUSE LEAVE! - Setting timeout');
    ...
    console.log('ProfileDropdown render - isUserMenuOpen:', isUserMenuOpen);
    ...
    return (
        <div
            onMouseEnter={() => {
                console.log('DROPDOWN MOUSE ENTER! - Direct event');
                handleMouseEnter();
            }}
            onMouseLeave={() => {
                console.log('DROPDOWN MOUSE LEAVE! - Direct event');
                handleMouseLeave();
            }}
            className="fixed top-16 right-4 w-80 bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl z-50 overflow-hidden"
            style={{
            ...
            <div className="p-4 space-y-2">
                ...
                {((user?.roles?.includes('admin') || user?.roles?.includes('super-admin'))) && (
                    <button
                        onClick={() => {
                            ...
                            console.log('Add Blog Post clicked');
                            setIsUserMenuOpen(false);
                        }}
                        className="w-full flex items-center gap-3 p-3 text-white hover:bg-white/10 rounded-xl transition-all duration-200 group"
                    >
                ...
                <button
                    onClick={async () => {
                        ...
                        try {
                            await logout();
                            console.log('✅ ProfileDropdown Logout successful');
                            navigateTo('home');
                        } catch (error) {
...
const ProductsPage = ({ navigateTo, showProductManagement, setShowProductManagement, setOpenAddForm }: { navigateTo: (page: string, params?: any) => void, showProductManagement: boolean, setShowProductManagement: (value: boolean) => void, setOpenAddForm: (value: boolean) => void }) => {
    console.log("🛍️ ProductsPage: Component rendered with showProductManagement:", showProductManagement);
    const { user } = useAuth();
    ...
    console.log("🛍️ ProductsPage: isAuthorized:", isAuthorized, "user:", user, "roles:", user?.roles, "loading:", loading);
    
    // Fetch products from database - INSTANT LOADING
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                console.log("🛍️ ProductsPage: Fetching products...");
                ...
                console.log("✅ ProductsPage: Products loaded:", formattedProducts.length);
            } catch (error) {
        ...
        <div className="min-h-screen bg-dark text-white">
            ...
            <div className="container mx-auto px-4 relative z-10">
                    <div className="max-w-4xl mx-auto text-center">
                        ...
                        {isAuthorized && (
                            console.log("🟢 ProductsPage: Rendering authorized buttons"),
                            <div className="flex justify-center gap-4 mb-8">
                                ...
                                <button
                                    onClick={() => {
                                        ...
                                        console.log("🟢 Setting openAddForm to true");
                                        console.log("🟢 Setting showProductManagement to true");
                                        setOpenAddForm(true);
                                        setShowProductManagement(true);
                                    }}
                            ...
                            <div className="flex flex-col items-center gap-4">
                                ...
                                {isAuthorized && (
                                    <div className="flex gap-4 mt-6">
                                        <button
                                            onClick={() => {
                                                console.log("🔵 Manage Products button clicked (loading state)!");
                                                setShowProductManagement(true);
                                            }}
                                        ...
                                        <button
                                            onClick={() => {
                                                console.log("🟢 Add New Product button clicked (loading state)!");
                                                console.log("🟢 Setting openAddForm to true");
                                                console.log("🟢 Setting showProductManagement to true");
                                                setOpenAddForm(true);
                                                setShowProductManagement(true);
        ...
        const fetchBlogPosts = async () => {
            try {
                ...
                const formattedPosts = data?.map(post => ({
                    ...
                    imageUrl: post.featured_image || 'https://images.unsplash.com/photo-1558655146-364adaf1fcc9?w=400&h=400&fit=crop&crop=center',
                ...
                setLoading(false); // Set to false immediately
                console.log("✅ BlogPage: Blog posts loaded:", formattedPosts.length);
            } catch (error) {
                            ...
                            <div className="flex justify-center mb-8">
                                <button
                                    onClick={() => {
                                        ...
                                        console.log('Add Blog Post clicked from Blog page');
                                    }}
                    ...
                    {loading ? (
                        ...
                        <div className="flex justify-center items-center py-20">
                            <div className="text-center">
                                ...
                                {isAuthorized && (
                                    <button
                                        onClick={() => console.log('Add first blog post')}
...
const App = () => {
    ...
    // Debug state changes
    React.useEffect(() => {
        console.log("🔄 App: showProductManagement changed to:", showProductManagement);
    }, [showProductManagement]);
    
    React.useEffect(() => {
        console.log("🔄 App: openAddForm changed to:", openAddForm);
    }, [openAddForm]);
    ...
    // Global data states
    // Removed global data states - each page manages its own data

    // Skip connection test for now - just set as connected
    React.useEffect(() => {
        console.log("🔍 App: Skipping connection test, assuming connected");
        setConnectionStatus('connected');
    }, []);

    // Removed global data loading - each page fetches its own data
    ...
    React.useEffect(() => {
        const handleProfileClick = () => {
            console.log('ProfileClick event received!', isUserMenuOpen);
    ...
    const navigateTo = (page: string, params?: any) => {
        ...
        const renderPage = () => {
            ...
            switch (currentPage) {
                ...
                case 'products':
                    console.log("🛍️ Rendering ProductsPage with showProductManagement:", showProductManagement);
        ...
        // REMOVED LOADING SCREEN - INSTANT LOADING
    ...
    return (
        <div className="min-h-screen bg-dark text-white">
            ...
            {showProductManagement && (
                console.log("🟢 App: Rendering ProductManagement modal with openAddForm:", openAddForm),
                <ProductManagement 
                    onClose={() => {
                        console.log("🔴 App: Closing ProductManagement modal");
